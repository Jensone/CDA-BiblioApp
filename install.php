...
/**
 * Page d'installation de la base de données
 */
...
// Ajout des tables 
$queryTables = Database::connect()->prepare("
 CREATE TABLE book ( id INT PRIMARY KEY AUTO_INCREMENT, title VARCHAR(255), author VARCHAR(255), edition VARCHAR(255), isbn VARCHAR(255), category VARCHAR(255), pages INT, format VARCHAR(255));
 CREATE TABLE client ( id INT PRIMARY KEY AUTO_INCREMENT, firstname VARCHAR(255), lastname VARCHAR(255), email VARCHAR(255), phone VARCHAR(255), birthdayYear INT, address VARCHAR(255), city VARCHAR(255), deposit BOOLEAN, bookings JSON);
 CREATE TABLE booking ( id INT PRIMARY KEY AUTO_INCREMENT, client INT, book INT, dateStart DATE, dateEnd DATE);
 ");
...
$queryInsert = Database::connect()->prepare("
...
if ($queryTables && $queryInsert) {
    echo "Youpi ! Sounds good !";
} else {
